    ...
    override fun addCosts(costs: CostMap, server: MinecraftServer) {
        ...
        for (recipe in recipeManager.values()) {
            if (recipe is RebornRecipe) {
                ...
                logger.info(
                    "TechReborn recipe for %s called %s (%s)",
                    recipe.outputs.map {
                        var s = it.toString()
                        if (it.tag != null) {
                            s += " ${it.tag}"
                        ...
                        s
                    }, recipe.id,recipe.javaClass.simpleName)
                ...
                if (recipe is RebornFluidRecipe) {
                    ...
                    val cost = costs[stack]
                    // The raw value is the only number I can get, what do you mean "Deprecated"
                    @Suppress("DEPRECATION")
                    totalCost.add(listOf(Pair(cost, fluid.amount.rawValue)))
                }
                ...
                for (output in recipe.outputs) {
                    ...
                    logger.info("    %d/%d %s", multiplier, outputCount, stack)
                    val cost = costs[stack]
